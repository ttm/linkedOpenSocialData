...
\begin{enumerate}[leftmargin=0cm]
	...
	\item Retrieve the number of relations, be them interactions or
            ...
            \textttt{ SELECT (COUNT(?interaction) as ?c) WHERE \{ \{
                    ?interaction a po:Friendship \} UNION \{ ?interaction
                    a po:Interaction \} UNION \{ ?interaction po:retweetOf
	...
	\item Retrieve all text produced by an specific user:\\
            \textttt{SELECT (CONCAT(?text) as ?texts) WHERE \{ ?activity
            po:author <user\_uri> . ?activity po:text ?text . \}}
	\item List users with the most friendships and the number of
            friendships in descending order by the number of friendships:\\
            \textttt{SELECT DISTINCT ?participant (COUNT(?interaction)
            as ?c) WHERE \{ ?interaction a po:Friendship . ?interaction
        po:member ?participant . \} ORDER BY DESC(?c)}
	\item Search string ``pineapple'' in LOSD messages:\\
            \textttt{SELECT ?text WHERE \{ ?activity po:text ?text .
            FILTER regex(?text, 'pineapple', 'i') \}}
\end{enumerate}
